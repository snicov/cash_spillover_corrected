...
version 11
syntax varlist(ts fv min=2) [if] [in] [fw iw pw aw/], ///
...
quietly: reg `Y' `X' [`weight' = `exp'] if `touse', nocons
estimates store OLS
...
for (ti = 1; ti <= Ntime; ti++){
	...
	for (i = 1; i <=n1; i++){
        ...
        // step b: construct X'e'eX for the given observation
 		...
 		XeeXh = ((X1[i,.]'*J(1,n1,1)*e1[i,1]*w_dia1[i,i]):*(J(k,1,1)*e1'*w_dia1:*window_i'))*X1 
...
for (pi = 1; pi <= Npanel; pi++){
	...
	for (t = 1; t <=n1; t++){
        ...
        // step b: construct X'e'eX for given observation
 		...
 		XeeXh = ((X1[t,.]'*J(1,n1,1)*e1[t,1]*w_dia1[t,t]):*(J(k,1,1)*e1'*w_dia1:*window_t'))*X1	
