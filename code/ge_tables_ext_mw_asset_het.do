...
cap program drop table_main_ext_mw_asset_het
program define table_main_ext_mw_asset_het
...
// Adding baseline variables - if they are in the dataset
        ...
        if (_rc == 0) {
		  ...
		  ** MiWi: interact with asset indicator
  ...
  if $runGPS == 0 {
	...
	// Setting up weights for control eligibles in ineligibles
		...
		// Get mean total spillover effect on above-median-asset households
				...
				sum `vrb' [aweight=weight] if ((eligible == 0 | treat == 0) & (below_p50_asset==0))
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col3 col4 col5 col6 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Non-recipient households}", pattern(1 0 0 1) ///
	prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mtitle("\shortstack{Total Effect \\ IV}" "\shortstack{Below-median \\ assets}" "\shortstack{Above-median \\ assets}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
