...
* heterogeneity by baseline assets, and decompose effects by Below X Treatment
...
cap program drop table_main_ext_mw_asset_split
program define table_main_ext_mw_asset_split
...
// Adding baseline variables - if they are in the dataset
        ...
        if (_rc == 0) {
          ...
          cap gen `v'_BL_e = below_p50_asset * `v'_BL
          cap gen M`v'_BL_e = below_p50_asset * M`v'_BL
...
** village-level
  ...
  local r2 = `r' - 2
	...
	// For each household, one of the two will be zero
				gen pp_actamt_`r2'to`rad'km_el = pp_actamt_`r2'to`rad'km * below_p50_asset
				gen pp_actamt_`r2'to`rad'km_in = pp_actamt_`r2'to`rad'km * above_p50_asset
	// Interact exogenous regressors with eligbiility status
	* el = below-med
	* in = above-med
				gen share_ge_elig_treat_`r2'to`rad'km_el = share_ge_elig_treat_`r2'to`rad'km * below_p50_asset
				gen share_ge_elig_treat_`r2'to`rad'km_in = share_ge_elig_treat_`r2'to`rad'km * above_p50_asset
  ...
  if $runGPS == 0 {
    ivreg2 `v' below_p50_asset `blvars_untreat' (`endregs' = `exregs')  [aweight=weight] if (eligible == 0 | treat == 0), cluster(sublocation_code)
  }

			loc firststage_p `e(idp)'

	// Setting up weights for control eligibles in ineligibles
            **MiWi: below-median assets and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_asset==1))
            local mean1 = r(sum)
            **MiWi: nonrecipient and below-median assets and treatment
			sum weight if ((eligible == 0 & treat == 1) & (below_p50_asset==1))
            local mean2 = r(sum)
            **MiWi: above-median assets and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_asset==0))
			local mean3 = r(sum)
            **MiWi: nonrecipient and above-median assets and treatment
		...
		// To look at three ATE's
			...
			foreach vrb of local amount_list {
                ...
                * use `vrb'_el for below-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_asset==1))
                local amt1 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`belowmed_cont_weight'" + "*`amt1'" + "*`vrb'_el"

		// Get mean total spillover effect on below-median-asset control households
		...
		// Get mean total spillover effect on below-median-asset treatment households
				local ATEstring_belowmed_treat_spill = "`ATEstring_belowmed_treat_spill'" + "+" + "`amt2'" + "*`vrb'_el"

                **MiWi: above-control
                * use `vrb'_in for above-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_asset==0))
                local amt3 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`abovemed_cont_weight'" + "*`amt3'" + "*`vrb'_in"

		// Get mean total spillover effect on above-median-asset control households
		...
		// Get mean total spillover effect on above-median-asset treatment households
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	mtitle("\shortstack{Total Effect \\ IV}" "\shortstack{Below-median \\ assets, control}" "\shortstack{Below-median \\ assets, treatment}" "\shortstack{Above-median \\ assets, control}" "\shortstack{Above-median \\ assets, treatment}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
