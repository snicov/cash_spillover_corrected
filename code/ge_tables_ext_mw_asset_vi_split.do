...
cap program drop table_main_ext_mw_asset_vi_split
program define table_main_ext_mw_asset_vi_split
  ...
  if $runGPS == 0 {
	...
	// Setting up weights for control eligibles in ineligibles
            **MiWi: below-median assets and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_asset_vill==1))
            local mean1 = r(sum)
            **MiWi: nonrecipient and below-median assets and treatment
			sum weight if ((eligible == 0 & treat == 1) & (below_p50_asset_vill==1))
            local mean2 = r(sum)
            **MiWi: above-median assets and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_asset_vill==0))
			local mean3 = r(sum)
            **MiWi: nonrecipient and above-median assets and treatment
			sum weight if ((eligible == 0 & treat == 1) & (below_p50_asset_vill==0))
			local mean4 = r(sum)
		...
		// To look at three ATE's
			...
			foreach vrb of local amount_list {
                ...
                * use `vrb'_el for below-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_asset_vill==1))
                local amt1 = r(mean)
		...
		// Get mean total spillover effect on below-median-asset control households
                ...
                **MiWi: below-treatment
                ...
                sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p50_asset_vill==1))
                local amt2 = r(mean)
		...
		// Get mean total spillover effect on below-median-asset treatment households
                ...
                * use `vrb'_in for above-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_asset_vill==0))
                local amt3 = r(mean)
		...
		// Get mean total spillover effect on above-median-asset control households
                ...
                **MiWi: above-treatment
                sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p50_asset_vill==0))
                local amt4 = r(mean)
