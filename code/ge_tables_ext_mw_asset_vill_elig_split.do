...
cap program drop table_main_ext_mw_ass_v_el_split
program define table_main_ext_mw_ass_v_el_split
...
// Adding baseline variables - if they are in the dataset
        ...
        if (_rc == 0) {
		  ...
		  cap gen `v'_BL_a = below_p50_asset_vill * `v'_BL
          cap gen M`v'_BL_a = below_p50_asset_vill * M`v'_BL
          cap gen `v'_BLae = below_p50_asset_vill * eligible * `v'_BL
          cap gen M`v'_BLae = below_p50_asset_vill * eligible * M`v'_BL
...
** village-level
  ...
  local r2 = `r' - 2
	...
	// For each household, one of the two will be zero
				gen pp_actamt_`r2'to`rad'km_el_be = pp_actamt_`r2'to`rad'km * below_p50_asset_vill * eligible
				gen pp_actamt_`r2'to`rad'km_el_ab = pp_actamt_`r2'to`rad'km * above_p50_asset_vill * eligible
				gen pp_actamt_`r2'to`rad'km_in_be = pp_actamt_`r2'to`rad'km * below_p50_asset_vill * ineligible
				gen pp_actamt_`r2'to`rad'km_in_ab = pp_actamt_`r2'to`rad'km * above_p50_asset_vill * ineligible
	// Interact exogenous regressors with eligbiility status
				gen share_ge_elig_treat_`r2'to`rad'km_el_be = share_ge_elig_treat_`r2'to`rad'km * below_p50_asset_vill * eligible
				gen share_ge_elig_treat_`r2'to`rad'km_el_ab = share_ge_elig_treat_`r2'to`rad'km * above_p50_asset_vill * eligible
				gen share_ge_elig_treat_`r2'to`rad'km_in_be = share_ge_elig_treat_`r2'to`rad'km * below_p50_asset_vill * ineligible
				gen share_ge_elig_treat_`r2'to`rad'km_in_ab = share_ge_elig_treat_`r2'to`rad'km * above_p50_asset_vill * ineligible
  ...
  if $runGPS == 0 {
    ivreg2 `v' below_p50_asset_vill eligible belowXelig_vill `blvars_untreat' (`endregs' = `exregs')  [aweight=weight] if (eligible == 0 | treat == 0), cluster(sublocation_code)
  }
	...
	// Setting up weights for control eligibles in ineligibles
            **MiWi: below-median assets and control and eligible
			sum weight if ((eligible == 1 & treat == 0) & (below_p50_asset_vill==1))
            local mean1 = r(sum)
            ...
            **MiWi: above-median assets and control and eligible
			sum weight if ((eligible == 1 & treat == 0) & (below_p50_asset_vill==0))
            local mean3 = r(sum)
		...
		// To look at three ATE's
			...
			foreach vrb of local amount_list {
                **MiWi: below-control-elig
				sum `vrb' [weight=weight] if ((eligible == 1 & treat == 0) & (below_p50_asset_vill==1))
                local amt1 = r(mean)
                ...
                **MiWi: above-control-elig
				sum `vrb' [weight=weight] if ((eligible == 1 & treat == 0) & (below_p50_asset_vill==0))
                local amt3 = r(mean)
                ...
                **MiWi: above-control-inelig
				sum `vrb' [weight=weight] if ((eligible == 0 & treat == 0) & (below_p50_asset_vill==0))
                local amt4 = r(mean)
