**MiWi: Table B8 using Amt X 1{below-median business income} instead of Amt X Elig
* heterogeneity by baseline business income, and decompose effects by Below X Treatment
    * below-treat, below-control, above-treat, above-control
...
cap program drop table_main_ext_mw_biz_split
program define table_main_ext_mw_biz_split
...
// Adding baseline variables - if they are in the dataset
        ...
        if (_rc == 0) {
          ...
          cap gen `v'_BL_e = below_p50_biz * `v'_BL
          cap gen M`v'_BL_e = below_p50_biz * M`v'_BL
...
** village-level
  ...
  local r2 = `r' - 2
	...
	// Interact endogenous regressors with eligibility status; MiWi: use baseline business revenue
	// For each household, one of the two will be zero
				gen pp_actamt_`r2'to`rad'km_el = pp_actamt_`r2'to`rad'km * below_p50_biz
				gen pp_actamt_`r2'to`rad'km_in = pp_actamt_`r2'to`rad'km * above_p50_biz
	// Interact exogenous regressors with eligbiility status
	* el = below-med
	* in = above-med
				gen share_ge_elig_treat_`r2'to`rad'km_el = share_ge_elig_treat_`r2'to`rad'km * below_p50_biz
				gen share_ge_elig_treat_`r2'to`rad'km_in = share_ge_elig_treat_`r2'to`rad'km * above_p50_biz
  ...
  if $runGPS == 0 {
    ivreg2 `v' below_p50_biz `blvars_untreat' (`endregs' = `exregs')  [aweight=weight] if (eligible == 0 | treat == 0), cluster(sublocation_code)
  }

			loc firststage_p `e(idp)'

	// Setting up weights for control eligibles in ineligibles
            **MiWi: below-median revenue and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_biz==1))
            local mean1 = r(sum)
            **MiWi: nonrecipient and below-median revenue and treatment
			sum weight if ((eligible == 0 & treat == 1) & (below_p50_biz==1))
            local mean2 = r(sum)
            **MiWi: above-median revenue and control (both eligible and ineligible)
			sum weight if ((treat == 0) & (below_p50_biz==0))
			local mean3 = r(sum)
            **MiWi: nonrecipient and above-median revenue and treatment
			sum weight if ((eligible == 0 & treat == 1) & (below_p50_biz==0))
			local mean4 = r(sum)

			local belowmed_cont_weight = `mean1' / (`mean1' + `mean2' + `mean3' + `mean4')
			local belowmed_treat_weight = `mean2' / (`mean1' + `mean2' + `mean3' + `mean4')
			local abovemed_cont_weight = `mean3' / (`mean1' + `mean2' + `mean3' + `mean4')
			local abovemed_treat_weight = `mean4' / (`mean1' + `mean2' + `mean3' + `mean4')

		// To look at three ATE's
			local ATEstring_tot_spill = "0"
			local ATEstring_belowmed_cont_spill = "0"
			local ATEstring_belowmed_treat_spill = "0"
			local ATEstring_abovemed_cont_spill = "0"
			local ATEstring_abovemed_treat_spill = "0"

			foreach vrb of local amount_list {
				/* sum `vrb' [weight=weight] if (eligible == 1 & treat == 0) */
                **MiWi: below-control
                    * both eligible and ineligible
                * use `vrb'_el for below-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_biz==1))
                local amt1 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`belowmed_cont_weight'" + "*`amt1'" + "*`vrb'_el"

		// Get mean total spillover effect on below-median-revenue control households
				local ATEstring_belowmed_cont_spill = "`ATEstring_belowmed_cont_spill'" + "+" + "`amt1'" + "*`vrb'_el"

                **MiWi: below-treatment
                    * need to exclude recipients (treatment-eligible)
                sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p50_biz==1))
                local amt2 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`belowmed_treat_weight'" + "*`amt2'" + "*`vrb'_el"

		// Get mean total spillover effect on below-median-revenue treatment households
				local ATEstring_belowmed_treat_spill = "`ATEstring_belowmed_treat_spill'" + "+" + "`amt2'" + "*`vrb'_el"

                **MiWi: above-control
                * use `vrb'_in for above-median
				sum `vrb' [weight=weight] if ((treat == 0) & (below_p50_biz==0))
                local amt3 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`abovemed_cont_weight'" + "*`amt3'" + "*`vrb'_in"

		// Get mean total spillover effect on above-median-revenue control households
				local ATEstring_abovemed_cont_spill = "`ATEstring_abovemed_cont_spill'" + "+" + "`amt3'" + "*`vrb'_in"

                **MiWi: above-treatment
                sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p50_biz==0))
                local amt4 = r(mean)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`abovemed_treat_weight'" + "*`amt4'" + "*`vrb'_in"

		// Get mean total spillover effect on above-median-revenue treatment households
				local ATEstring_abovemed_treat_spill = "`ATEstring_abovemed_treat_spill'" + "+" + "`amt4'" + "*`vrb'_in"
      ...
      pstar, b(`r(estimate)') se(`r(se)')
			...
			disp "`ATEstring_belowmed_cont_spill'"
			lincom "`ATEstring_belowmed_cont_spill'"

      pstar, b(`r(estimate)') se(`r(se)')
			estadd local thisstat`count' = "`r(bstar)'": col4
			estadd local thisstat`countse' = "`r(sestar)'": col4

			disp "`ATEstring_belowmed_treat_spill'"
			lincom "`ATEstring_belowmed_treat_spill'"

      pstar, b(`r(estimate)') se(`r(se)')
			estadd local thisstat`count' = "`r(bstar)'": col5
			estadd local thisstat`countse' = "`r(sestar)'": col5

			disp "`ATEstring_abovemed_cont_spill'"
			lincom "`ATEstring_abovemed_cont_spill'"
    di "test, finished col5"

      pstar, b(`r(estimate)') se(`r(se)')
			estadd local thisstat`count' = "`r(bstar)'": col6
			estadd local thisstat`countse' = "`r(sestar)'": col6

            disp "`ATEstring_abovemed_treat_spill'"
    di "test, finished col6"
			lincom "`ATEstring_abovemed_treat_spill'"
    ...
    di "test, finished col7"

** 6. Sixth column: Add control, low sat mean of dependent variable **
**********************************************************************
		sum `v' [weight=weight] if treat == 0 & hi_sat == 0

		estadd local thisstat`count' = string(`r(mean)', "%9.2f") : col8
		estadd local thisstat`countse' = "(" + string(`r(sd)', "%9.2f") + ")": col8
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col3 col4 col5 col6 col7 col8 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Non-recipient households}", pattern(1 0 0 0 0 1) ///
	prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mtitle("\shortstack{Total Effect \\ IV}" "\shortstack{Below-median \\ revenue, control}" "\shortstack{Below-median \\ revenue, treatment}" "\shortstack{Above-median \\ revenue, control}" "\shortstack{Above-median \\ revenue, treatment}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
