**MiWi: decompose by control-inelig and treatment-inelig
...
cap program drop table_main_ext_mw_inelig
program define table_main_ext_mw_inelig
...
** village-level
  ...
  if $runGPS == 0 {
	...
	// Setting up weights for control eligibles in ineligibles
            ...
            **MiWi: control-eligible = control-ineligible
			sum weight if (eligible == 0 & treat == 0)
			local mean1 = r(sum)
            **MiWi: ineligible = treatment-ineligible
		...
		// To look at three ATE's
			...
			foreach vrb of local amount_list {
                ...
                **MiWi: control-inelig
				sum `vrb' [weight=weight] if (eligible == 0 | treat == 0)
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`eligcontrolweight'" + "*`r(mean)'" + "*`vrb'_in"
		// Get mean total spillover effect on control eligibles
				local ATEstring_eligcontrol_spill = "`ATEstring_eligcontrol_spill'" + "+" + "`r(mean)'" + "*`vrb'_in"
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	mtitle("\shortstack{$\mathds{1}(\text{Treat village})$ \\ \vspace{.1cm} \\ Reduced form }" "\shortstack{Total Effect \\ IV}" "\shortstack{Total Effect \\ IV}" "Control Ineligibles" "Treatment Ineligibles" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
