**MiWi: decompose by control-elig, control-inelig, and treatment-inelig
* ie. decompose Table B8 Column 5 (ineligibles) into control-inelig and treatment-inelig
...
cap program drop table_main_ext_mw_inelig_split
program define table_main_ext_mw_inelig_split
...
// Setting up blank table, 6 columns and as many rows as variables in `outcomes'
	...
	/* local ncols = 6 */
	local ncols = 7
    **MiWi: adding a column to split Ineligibles into treatment and control villages
  ...
  if $runGPS == 0 {
    ...
    **MiWi: three-way split: control-elig, control-inelig, treatment-inelig
        ...
        **MiWi: four
			...
			local ATEstring_ineligcontrol_spill = "0"
			local ATEstring_ineligtreatment_spill = "0"
			...
			foreach vrb of local amount_list {
                ...
                **MiWi: control-elig
				...
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`eligcontrolweight'" + "*`amt1'" + "*`vrb'_el"
		
        // Get mean total spillover effect on control eligibles
        		local ATEstring_eligcontrol_spill = "`ATEstring_eligcontrol_spill'" + "+" + "`amt1'" + "*`vrb'_el"
		
        // Get mean total spillover effect on ineligibles
				/* sum `vrb' [aweight=weight] if eligible == 0 */
                **MiWi: control-ineligible
				sum `vrb' [aweight=weight] if (eligible == 0 & treat == 0)
                local amt2 = r(mean)

				local ATEstring_ineligcontrol_spill = "`ATEstring_ineligcontrol_spill'" + "+" + "`amt2'" + "*`vrb'_in"
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`ineligcontrolweight'" + "*`amt2'" + "*`vrb'_in"
             
                **MiWi: treatment-ineligible
				sum `vrb' [aweight=weight] if (eligible == 0 & treat == 1)
                local amt3 = r(mean)
				local ATEstring_ineligtreatment_spill = "`ATEstring_ineligtreatment_spill'" + "+" + "`amt3'" + "*`vrb'_in"
                
		// Get mean total spillover effect on all untreated households
				local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`ineligtreatmentweight'" + "*`amt3'" + "*`vrb'_in"
				/* local ATEstring_tot_spill = "`ATEstring_tot_spill'" + "+" + "`ineligcontrolweight'" + "*`amt2'" + "*`vrb'_in" + "+" + "`ineligtreatmentweight'" + "*`amt3'" + "*`vrb'_in" */
			}
      ...
      pstar, b(`r(estimate)') se(`r(se)')
			...
			disp "`ATEstring_ineligcontrol_spill'"
			lincom "`ATEstring_ineligcontrol_spill'"

      pstar, b(`r(estimate)') se(`r(se)')
			estadd local thisstat`count' = "`r(bstar)'": col5
			estadd local thisstat`countse' = "`r(sestar)'": col5

            disp "`ATEstring_ineligtreatment_spill'"
			lincom "`ATEstring_ineligtreatment_spill'"
...
**********************************************************************
		...
		estadd local thisstat`count' = string(`r(mean)', "%9.2f") : col7
		estadd local thisstat`countse' = "(" + string(`r(sd)', "%9.2f") + ")": col7
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col1 col2 col3 col4 col5 col6 col7 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Recipient households}" "\textbf{Non-recipient households}", pattern(1 0 1 0 0 0 1) ///
	prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mtitle("\shortstack{$\mathds{1}(\text{Treat village})$ \\ \vspace{.1cm} \\ Reduced form }" "\shortstack{Total Effect \\ IV}" "\shortstack{Total Effect \\ IV}" "\shortstack{Control \\ Eligibles}" "\shortstack{Control \\ Ineligibles}" "\shortstack{Treatment \\ Ineligibles}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
