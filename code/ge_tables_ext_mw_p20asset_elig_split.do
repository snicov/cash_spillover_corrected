...
cap program drop table_main_mw_p20ass_elig_split
program define table_main_mw_p20ass_elig_split
...
// Adding baseline variables - if they are in the dataset
        ...
        if (_rc == 0) {
          ...
          cap gen `v'_BL_a = below_p20_asset * `v'_BL
          cap gen M`v'_BL_a = below_p20_asset * M`v'_BL
          cap gen `v'_BLae = below_p20_asset * eligible * `v'_BL
          cap gen M`v'_BLae = below_p20_asset * eligible * M`v'_BL
...
** village-level
  ...
  local r2 = `r' - 2
	...
	// For each household, one of the two will be zero
				gen pp_actamt_`r2'to`rad'km_el_be = pp_actamt_`r2'to`rad'km * below_p20_asset * eligible
				gen pp_actamt_`r2'to`rad'km_el_ab = pp_actamt_`r2'to`rad'km * above_p20_asset * eligible
				gen pp_actamt_`r2'to`rad'km_in_be = pp_actamt_`r2'to`rad'km * below_p20_asset * ineligible
				gen pp_actamt_`r2'to`rad'km_in_ab = pp_actamt_`r2'to`rad'km * above_p20_asset * ineligible
	// Interact exogenous regressors with eligbiility status
				gen share_ge_elig_treat_`r2'to`rad'km_el_be = share_ge_elig_treat_`r2'to`rad'km * below_p20_asset * eligible
				gen share_ge_elig_treat_`r2'to`rad'km_el_ab = share_ge_elig_treat_`r2'to`rad'km * above_p20_asset * eligible
				gen share_ge_elig_treat_`r2'to`rad'km_in_be = share_ge_elig_treat_`r2'to`rad'km * below_p20_asset * ineligible
				gen share_ge_elig_treat_`r2'to`rad'km_in_ab = share_ge_elig_treat_`r2'to`rad'km * above_p20_asset * ineligible
  ...
  if $runGPS == 0 {
    ivreg2 `v' below_p20_asset eligible below_p20Xelig `blvars_untreat' (`endregs' = `exregs')  [aweight=weight] if (eligible == 0 | treat == 0), cluster(sublocation_code)
  }
	...
	// Setting up weights for control eligibles in ineligibles
            **MiWi: below-median assets and control and eligible
			sum weight if ((eligible == 1 & treat == 0) & (below_p20_asset==1))
            local mean1 = r(sum)
            **MiWi: below-median assets and control and ineligible
			sum weight if ((eligible == 0 & treat == 0) & (below_p20_asset==1))
            local mean2 = r(sum)
            **MiWi: above-median assets and control and eligible
			sum weight if ((eligible == 1 & treat == 0) & (below_p20_asset==0))
            local mean3 = r(sum)
            **MiWi: above-median assets and control and ineligible
			sum weight if ((eligible == 0 & treat == 0) & (below_p20_asset==0))
            local mean4 = r(sum)
            **MiWi: below-median assets and treatment and ineligible
			sum weight if ((eligible == 0 & treat == 1) & (below_p20_asset==1))
            local mean5 = r(sum)
            **MiWi: above-median assets and treatment and ineligible
			sum weight if ((eligible == 0 & treat == 1) & (below_p20_asset==0))
            local mean6 = r(sum)
		...
		// To look at three ATE's
			...
			foreach vrb of local amount_list {
                **MiWi: below-control-elig
				sum `vrb' [weight=weight] if ((eligible == 1 & treat == 0) & (below_p20_asset==1))
                local amt1 = r(mean)
                ...
                **MiWi: below-control-inelig
				sum `vrb' [weight=weight] if ((eligible == 0 & treat == 0) & (below_p20_asset==1))
                local amt2 = r(mean)
                ...
                **MiWi: above-control-elig
				sum `vrb' [weight=weight] if ((eligible == 1 & treat == 0) & (below_p20_asset==0))
                local amt3 = r(mean)
                ...
                **MiWi: above-control-inelig
				sum `vrb' [weight=weight] if ((eligible == 0 & treat == 0) & (below_p20_asset==0))
                local amt4 = r(mean)
                ...
                **MiWi: below-treatment-inelig
				sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p20_asset==1))
                local amt5 = r(mean)
                ...
                **MiWi: above-treatment-inelig
				sum `vrb' [weight=weight] if ((eligible == 0 & treat == 1) & (below_p20_asset==0))
                local amt6 = r(mean)
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	mtitle("\shortstack{Total Effect \\ IV}" "\shortstack{Below-p20 \\ assets, control \\ eligibles}" "\shortstack{Below-p20 \\ assets, control \\ ineligibles}" "\shortstack{Below-p20 \\ assets, treatment \\ ineligibles}" "\shortstack{Above-p20 \\ assets, control \\ eligibles}" "\shortstack{Above-p20 \\ assets, control \\ ineligibles}" "\shortstack{Above-p20 \\ assets, treatment \\ ineligibles}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
