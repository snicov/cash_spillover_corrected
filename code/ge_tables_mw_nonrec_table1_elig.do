...
cap program drop table_main_mw_nonrec_t1_elig
program define table_main_mw_nonrec_t1_elig
...
** village- level **
    ...
    *** Estimating total effects: IV with spatially correlated standard errors ***
    ...
    local endregs "pp_actamt_ownvill_el pp_actamt_ownvill_in"
    /* local endregs "pp_actamt_ownvill_in" */
    ...
    forval rad = 2(2)`r' {
      ...
      * interact other-village cash with eligible, ineligible
        * own-village is already specific to treatment-ineligibles
      gen pp_actamt_ownvill_in = pp_actamt_ownvill * ineligible
      gen pp_actamt_ownvill_el = pp_actamt_ownvill * eligible

      gen pp_actamt_ov_`r2'to`rad'km_el = pp_actamt_ov_`r2'to`rad'km * eligible
      gen pp_actamt_ov_`r2'to`rad'km_in = pp_actamt_ov_`r2'to`rad'km * ineligible
      // Interact exogenous regressors with eligbiility status
      gen share_ge_elig_treat_ov_`r2'to`rad'km_el = share_ge_elig_treat_ov_`r2'to`rad'km * eligible
      gen share_ge_elig_treat_ov_`r2'to`rad'km_in = share_ge_elig_treat_ov_`r2'to`rad'km * ineligible

      local endregs "`endregs' pp_actamt_ov_`r2'to`rad'km_el pp_actamt_ov_`r2'to`rad'km_in"
      local endregs_other "`endregs_other' pp_actamt_ov_`r2'to`rad'km"
      local exregs "`exregs' share_ge_elig_treat_ov_`r2'to`rad'km_el share_ge_elig_treat_ov_`r2'to`rad'km_in"
    ...
    if $runGPS == 0 {
      ivreg2 `v' eligible `blvars_untreat' (`endregs' = `exregs') [aweight=weight] if sample == 1, cluster(sublocation_code)
      **MiWi: use baseline vars interacted with eligible
    }
	...
	**MiWi: treatment-ineligible
    ...
    di "w1: `eligcontrolweight'"
    di "w2: `ineligcontrolweight'"
    di "w3: `ineligtreatmentweight'"
	...
	** MiWi: total own-village effect
      ...
      foreach var of local endregs_own {
		...
		* need to restrict to treatment-ineligs? controls have no own-village effect
        ...
        local ATEstring_own_ti = "`ATEstring_own_ti'" + "+" + "`r(mean)'" + "*" + "`var'_in"
        /* local ATEstring_own_ti = "`ATEstring_own_ti'" + "+" + "`r(mean)'" + "*" + "`var'" */

		/* local ATEstring_tot = "`ATEstring_tot'" + "+" + "`r(mean)'" + "*`var'_in" */
		/* local ATEstring_tot = "`ATEstring_tot'" + "+" + "*`r(mean)'" + "*`var'" */

        **MiWi: this shouldn't be weighted by population share; all weight is on treatment-ineligible?
		local ATEstring_tot = "`ATEstring_tot'" + "+" + "`ineligtreatmentweight'" + "*`r(mean)'" + "*`var'_in"
		...
		local ATEstring_tot = "`ATEstring_tot'" + "+" + "`ineligcontrolweight'" + "*`r(mean)'" + "*`var'_el"
		...
		local ATEstring_tot = "`ATEstring_tot'" + "+" + "`eligcontrolweight'" + "*`r(mean)'" + "*`var'_el"
	...
	* split by subgroup
      ...
      foreach var of local endregs_other {
        **MiWi: total effect on nonrecipients
        sum `var' [weight=weight]  if (eligible == 0 & treat==1)
        /* sum `var' [weight=weight]  if (sample == 1) */
        local ATEstring_other_ti = "`ATEstring_other_ti'" + "+" + "`r(mean)'" + "*" + "`var'_in"

		local ATEstring_tot = "`ATEstring_tot'" + "+" + "`ineligtreatmentweight'" + "*`r(mean)'" + "*`var'_in"

        sum `var' [weight=weight]  if (eligible == 0 & treat==0)
        local ATEstring_other_ci = "`ATEstring_other_ci'" + "+" + "`r(mean)'" + "*" + "`var'_in"

		local ATEstring_tot = "`ATEstring_tot'" + "+" + "`ineligcontrolweight'" + "*`r(mean)'" + "*`var'_in"

      	sum `var' [weight=weight]  if (eligible == 1 & treat==0)
        local ATEstring_other_ce = "`ATEstring_other_ce'" + "+" + "`r(mean)'" + "*" + "`var'_el"
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col3 col6 col7 col8 col9 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Non-recipient households}", pattern(1 0 0 0 0) ///
	prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mtitle("\shortstack{Own-village \\ Treat-inelig}" "\shortstack{Other-village \\ Treat-inelig}" "\shortstack{Other-village \\ Control-inelig}" "\shortstack{Other-village \\ Control-elig}" "\shortstack{Total Effect \\ IV}") ///
	stats(`statnames', labels(`varlabels')) note("`note'") prehead(`prehead') postfoot(`postfoot')

end
/* "\shortstack{Own-village \\ Control-inelig}" "\shortstack{Own-village \\ Control-elig}" */
