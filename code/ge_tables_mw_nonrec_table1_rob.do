...
cap program drop table_main_mw_nonrec_table1_rob
program define table_main_mw_nonrec_table1_rob
...
// For dummy regression, restrict sample to eligible households only
		...
		if inlist("`source'",  "$da/GE_HHLevel_ECMA.dta", "$da/GE_HHIndividualWageProfits_ECMA.dta") {
			/* gen sample = eligible */
            **MiWi: use non-recipients, drop outlier villages (cash transfer > 50% of per-capita GDP)
            gen sample = (eligible == 0 | treat == 0) & pp_actamt_ownvill < 0.5
		}
