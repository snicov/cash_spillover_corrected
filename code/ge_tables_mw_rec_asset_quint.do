...
cap program drop table_main_mw_rec_asset_quint
program define table_main_mw_rec_asset_quint
...
** village- level **
    ...
    *** Estimating total effects: IV with spatially correlated standard errors ***
    ...
    local endregs = ""
    local exregs = ""
    local amount_list = "pp_actamt_ownvill" // MiWi: use instead of endregs for non-interacted Amt terms

    ** interact own-village endogenous and exogenous regressors with baseline total asset quintile indicators
    forval i = 1/5 {
        gen pp_actamt_ownvill_q`i' = pp_actamt_ownvill * asset_q`i'
        gen treat_q`i' = treat * asset_q`i'
        local endregs = "`endregs' pp_actamt_ownvill_q`i'"
        local exregs = "`exregs' treat_q`i'"
    }
    ...
    forval rad = 2(2)`r' {
      ...
      ** interact endogenous and exogenous other-village regressors with asset quintile indicators
      forval i = 1/5 {
        gen pp_actamt_ov_`r2'to`rad'km_q`i' = pp_actamt_ov_`r2'to`rad'km * asset_q`i'
        gen share_ge_elig_treat_ov_`r2'to`rad'km_q`i' = share_ge_elig_treat_ov_`r2'to`rad'km * asset_q`i'
        local endregs "`endregs' pp_actamt_ov_`r2'to`rad'km_q`i'"
        local exregs "`exregs' share_ge_elig_treat_ov_`r2'to`rad'km_q`i'"
      }
    ...
    if $runGPS == 0 {
      ivreg2 `v' asset_q1 asset_q2 asset_q3 asset_q4 `blvars' (`endregs' = `exregs') [aweight=weight] if sample == 1, cluster(sublocation_code)
    }

	// Saving p-value of underidentification LM statistic as firststage_p
			loc firststage_p `e(idp)'

	// Setting up weights for quintiles
            forval i = 1/5 {
    			sum weight if (sample == 1 & (asset_q`i'==1))
    			local mean`i' = r(sum)
            }
            /* **MiWi: quintile1 
			sum weight if (sample == 1 & (asset_q1==1))
			local mean1 = r(sum)
            **MiWi: quintile2
			sum weight if (sample == 1 & (asset_q2==1))
			local mean2 = r(sum)
            **MiWi: quintile3 
			sum weight if (sample == 1 & (asset_q3==1))
			local mean3 = r(sum)
            **MiWi: quintile4
			sum weight if (sample == 1 & (asset_q4==1))
			local mean4 = r(sum)
            **MiWi: quintile5
			sum weight if (sample == 1 & (asset_q5==1))
			local mean5 = r(sum)
 */
        ...
        // To look at three ATE's
			...
			local ATEstring_q1 = "0"
			local ATEstring_q2 = "0"
			local ATEstring_q3 = "0"
			local ATEstring_q4 = "0"
			local ATEstring_q5 = "0"

      ** Get mean total effect on treated eligibles - loop through endogenous regressors **
      /* local ATEstring_tot = "0" */

      foreach var of local amount_list {
        forval i = 1/5 {
            sum `var' [weight=weight]  if (sample == 1 & treat == 1 & asset_q`i' == 1)
            local ATEstring_tot = "`ATEstring_tot'" + "+" + "`q`i'_weight'" +  "*`r(mean)'" + "*`var'_q`i'"
            local ATEstring_q`i' = "`ATEstring_q`i''" + "+" + "`r(mean)'" + "*`var'_q`i'"
...
** village-level
  ...
  local r2 = `r' - 2
	...
	// Interact endogenous regressors with eligibility status; MW: use baseline assets
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col2 col3 col4 col5 col6 col7 col8 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Recipient households}", pattern(1 0 0 0 0 0 1) ///
