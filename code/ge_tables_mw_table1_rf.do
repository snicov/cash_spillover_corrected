**MiWi: Table 1 reduced form: regress y on Treat and Share-eligible
...
cap program drop table_main_mw_table1_rf
program define table_main_mw_table1_rf
...
// For dummy regression, restrict sample to eligible households only
		...
		if inlist("`source'",  "$da/GE_HHLevel_ECMA.dta", "$da/GE_HHIndividualWageProfits_ECMA.dta") {
			gen sample = eligible
            **MiWi: drop outlier villages (cash transfer > 50% of per-capita GDP)
            /* gen sample = eligible == 1 & pp_actamt_ownvill < 0.5 */
		}
...
** village- level **
    ...
    *** Estimating total effects: IV with spatially correlated standard errors ***
    ...
    forval rad = 2(2)`r' {
      ...
      local exregs_other "`exregs_other' share_ge_elig_treat_ov_`r2'to`rad'km"
    }
    ...
    if $runGPS == 0 {
      reghdfe `v' `blvars' `exregs' [aweight=weight] if sample == 1, cluster(sublocation_code)
    ...
    **MiWi: report beta separately
	...
	// Saving p-value of underidentification LM statistic as firststage_p
      ...
      ** Get mean total effect on treated eligibles - loop through endogenous regressors **
      ...
      foreach var of local exregs {
        **MiWi: total effect on recipients
        sum `var' [weight=weight]  if (sample == 1 & treat == 1)
        local ATEstring_tot = "`ATEstring_tot'" + "+" + "`r(mean)'" + "*" + "`var'"
      }
    
    ** MiWi: total own-village effect on recipients
      local ATEstring_own = "0"
      foreach var of local exregs_own {
        sum `var' [weight=weight]  if (sample == 1 & treat == 1)
        local ATEstring_own = "`ATEstring_own'" + "+" + "`r(mean)'" + "*" + "`var'"
      }

    ** MiWi: total other-village effect, excluding own-village
      local ATEstring_other = "0"
      foreach var of local exregs_other {
        sum `var' [weight=weight]  if (sample == 1 & treat == 1)
        local ATEstring_other = "`ATEstring_other'" + "+" + "`r(mean)'" + "*" + "`var'"
      }

        **MiWi: other-village
			disp "`ATEstring_other'"
			lincom "`ATEstring_other'"
...
// Exporting tex table
  ...
  if $runGPS == 0 {
	...
	esttab col3 col4 col5 col7 `using', cells(none) booktabs nonotes compress replace ///
	mgroups("\textbf{Recipient households}", pattern(1 0 0 1) ///
	prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
	mtitle("Own-village" "Other-village" "\shortstack{Total Effect \\ IV}" "\shortstack{Control, low-saturation \\ mean (SD)}") ///
